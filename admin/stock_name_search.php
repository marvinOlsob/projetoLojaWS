...
$sql= "SELECT product.prodNam, product.prodId, product.prodCat, product.prodPho, product.prodSto, product.prodSup, product.prodPri, product.prodDes, category.cateNam, supplier.suppNam FROM product INNER JOIN category ON category.cateId = product.prodCat INNER JOIN supplier ON supplier.suppId = product.prodSup WHERE product.prodNam LIKE '%$_POST[stock_name]%' ORDER BY prodSto DESC";
...
if ($result->num_rows > 0){
        ...
        echo "
            <p>Products with '" . $_POST["stock_name"] . "':</p>
